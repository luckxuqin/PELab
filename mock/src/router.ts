...
if (`------------------------------------------------------- /api/get-excel`) {
  ...
  let workbook = XLSX.readFile(__dirname + '/PELabs.csv');
  ...
  const router = new Router()
      ...
      .put('/reserve', ctx => {
        ...
        XLSX.utils.sheet_add_aoa(workbook.Sheets[sheetNames[0]], [[item.team,item.name,item.project,item.startDate,item.endDate]], {origin: loc});
        XLSX.writeFile(workbook, __dirname + '/PELabs.csv');

        workbook = XLSX.readFile(__dirname + '/PELabs.csv');
